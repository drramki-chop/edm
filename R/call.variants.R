...
call.variants <- function(columnIndex,input){
  ...
  if (input.yaml$precomputed.controls){
    #################################### code for batch workflow with precomputed controls ####################################################
    ...
    # read batch counts
    batch <- readRDS(paste0(input.yaml$output.directory,"/results/",input.yaml$cohort.name,".exomedepth.cohort.auto.rds"))

    ### Insert the test sample
    cohort.object.auto$countmat <- cbind(cohort.object.auto$countmat,batch$countmat[,columnIndex,drop=FALSE])
    ...
    countmat.selected <- cohort.object.auto$countmat[cohort.object.auto$selected.exons,] ### change this
  ...
  if(sample_name %in% men){
    if (input.yaml$precomputed.controls){
      ...
      # read batch counts
      batch <- readRDS(paste0(input.yaml$output.directory,"/results/",input.yaml$cohort.name,".exomedepth.cohort.men.rds"))
      columnIndex <- which(colnames(batch$countmat) %in% sample_name)
      ### Insert the test sample
      cohort.object.x$countmat <- cbind(cohort.object.x$countmat,batch$countmat[,columnIndex,drop=FALSE])
      columnIndex = nControls + 1
    ...
    if (input.yaml$precomputed.controls){
      ...
      # read batch counts
      batch <- readRDS(paste0(input.yaml$output.directory,"/results/",input.yaml$cohort.name,".exomedepth.cohort.women.rds"))
      columnIndex <- which(colnames(batch$countmat) %in% sample_name)
      ### Insert the test sample
      cohort.object.x$countmat <- cbind(cohort.object.x$countmat,batch$countmat[,columnIndex,drop=FALSE])
      columnIndex = nControls + 1      
