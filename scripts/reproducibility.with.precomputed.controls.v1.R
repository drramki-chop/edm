...
## Uses an approximation to choose combination of controls (RcppAlgos)
...
correlations <- t(cor(exomedepth.object@test,countmat))

correlated.samples <- which(correlations >= 0.97)
if(length(correlated.samples) < 25){
  message("The number of controls correlated with the test sample is less than 25. Not proceeding with the reproducibility analysis.")
  stop()
}

correlated.samples.countmat <- countmat[,correlated.samples]
sample.mean <- mean(exomedepth.object@test)
reference.coverage.target <- sample.mean* 20

correlated.samples.colMeans <- colMeans(correlated.samples.countmat)
n.reference.to.choose <- round(reference.coverage.target/mean(correlated.samples.colMeans))

if(length(correlated.samples) < n.reference.to.choose + 5){
  message("The number of controls correlated with the test sample is less than required samples. Not proceeding with the reproducibility analysis. ")
  stop()
}

combinations <- as.data.frame(RcppAlgos::comboSample(c(1:length(correlated.samples)),n.reference.to.choose,n = 5000,repetition = F))
combinations$ratio <- apply(combinations,1,function(x){return(sample.mean/sum(correlated.samples.colMeans[as.numeric(x)]))})
combinations <- combinations[combinations$ratio <= 0.05 & combinations$ratio >=0.045,]
...
for (iter in 1:iterations) {
  ...
  reference_set = rowSums(correlated.samples.countmat[,as.numeric(combinations[iter,1:n.reference.to.choose])])
  
  loglike <- .Call(
    "get_loglike_matrix",
    phi = exomedepth.object@phi,
    expected = rep(combinations$ratio[iter],length(exomedepth.object@phi)),
